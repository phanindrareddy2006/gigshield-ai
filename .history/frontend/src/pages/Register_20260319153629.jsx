...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response
      if (isLogin) {
        response = await workerAPI.login({
          phone: formData.phone,
          password: formData.password,
        })
      } else {
        response = await workerAPI.register(formData)
      }
      ...
      setError(err.response?.data?.message || (isLogin ? 'Login failed. Please check credentials.' : 'Registration failed. Please try again.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100">
      <div className="card w-full max-w-md">
        <h1 className="text-3xl text-center mb-8 text-blue-600">{isLogin ? 'Worker Login' : 'Worker Registration'}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? 'Please wait...' : isLogin ? 'Login' : 'Register & Continue'}
          </button>
